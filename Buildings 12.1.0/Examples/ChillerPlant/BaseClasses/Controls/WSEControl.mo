...
model WSEControl "Control unit for WSE"
  ...
  Modelica.Blocks.Interfaces.RealInput wseCHWST(
    ...
    "WSE chilled water supply temperature (water entering WSE)" annotation (
      Placement(transformation(extent={{-60,80},{-20,120}}),
        iconTransformation(extent={{-60,80},{-20,120}})));
  ...
  DataCentersConfigurations.Controls.Chi_WSE.Modulation modulation
    annotation (Placement(transformation(extent={{60,140},{80,160}})));
  Modelica.Blocks.Interfaces.RealInput Architecture annotation (Placement(
...
equation
  ...
  connect(T2.outPort, off.inPort[1])   annotation (Line(
      ...
      color={0,0,0},
      smooth=Smooth.None));
  connect(feedback.y, y2)
    annotation (Line(points={{161,-40},{190,-40}}, color={0,0,127}));
  connect(const.y, feedback.u1)
    annotation (Line(points={{135,-40},{144,-40}}, color={0,0,127}));
  connect(Architecture, modulation.u) annotation (Line(points={{-40,160},{40,
          160},{40,150},{60,150}}, color={0,0,127}));
  connect(booToRea1.y, product1.u2)
    annotation (Line(points={{101,40},{114,40}}, color={0,0,127}));
  connect(product1.y, y1) annotation (Line(points={{137,46},{174,46},{174,40},{
          190,40}}, color={0,0,127}));
  connect(feedback.u2, product1.y) annotation (Line(points={{152,-32},{154,-32},
          {154,46},{137,46}}, color={0,0,127}));
  connect(modulation.WSEActivate, product1.u1) annotation (Line(points={{80.6,
          155.2},{106,155.2},{106,52},{114,52}}, color={0,0,127}));
  connect(off.active, booToRea1.u)
    annotation (Line(points={{10,76.8},{10,40},{78,40}}, color={255,0,255}));
  annotation (
